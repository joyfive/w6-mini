...
export const __checkId = createAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    try {
      const data = await axios.post(`${SERVICE_URL}/auth/checkid`, {
        userid: payload,
      })
...
export const __checkName = createAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    try {
      const data = await axios.post(`${SERVICE_URL}/auth/checkname`, {
        nickname: payload,
      })
      // 415는 타입에러. {}로 감싸서 보낸다.
      return thunkAPI.fulfillWithValue(data.data)
...
export const __userSignUp = createAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    try {
      const data = await axios.post(`${SERVICE_URL}/auth/signup`, payload)
      return thunkAPI.fulfillWithValue(data.data)
    } catch (error) {
      return thunkAPI.rejectWithValue(error)
    }
